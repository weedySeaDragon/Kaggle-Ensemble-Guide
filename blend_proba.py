...
def blend_proba(clf, X_train, y, X_test, nfolds=5, save_preds="",
    ...
    if len(save_preds) > 0:
        hashed_id = hashlib.md5.new("%s" % str(clf.get_params())).hexdigest() # orig: hashed_id =  (shadows existing var 'hashed_id')
        print("storing meta predictions at: %s" % save_preds)
        np.save("%s%s_%s_%s_train.npy" % (save_preds, clf_name, avg_loss, hashed_id), dataset_blend_train)
        np.save("%s%s_%s_%s_test.npy" % (save_preds, clf_name, avg_loss, hashed_id), dataset_blend_test)

    if len(save_test_only) > 0:
        hashed_id = hashlib.md5.new("%s" % str(clf.get_params())).hexdigest()
        print("storing meta predictions at: %s" % save_test_only)

        dataset_blend_test = clf.predict(X_test)
        np.savetxt("%s%s_%s_%s_test.txt" % (save_test_only, clf_name, avg_loss, hashed_id), dataset_blend_test)
        d = {}
        d["stacker"] = clf.get_params()
        d["generalizers"] = generalizers_params
        with open("%s%s_%s_%s_params.json" % (save_test_only, clf_name, avg_loss, hashed_id), 'wb') as f:
            json.dump(d, f)
    ...
    if len(save_params) > 0:
        ...
        d = {}
        d["name"] = clf_name
        d["params"] = {k: (v.get_params() if "\n" in str(v) or "<" in str(v) else v) for k, v in
                       clf.get_params().items()}
        d["generalizers"] = generalizers_params
        with open("%s%s_%s_%s_params.json" % (save_params, clf_name, avg_loss, hashed_id), 'wb') as f:
            json.dump(d, f)
